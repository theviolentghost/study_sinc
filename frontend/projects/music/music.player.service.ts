...
export class MusicPlayerService {
    ...
    public async load_track(data: Song_Identifier | Song_Data | string): Promise<void> {
        ...
        this.audio_data.current = {
            identifier: song_identifier,
            data: song_data || null, // if exists
            loaded: false,
            audio_source: '',
            source_type: null,
        };
        ...
        if(!song_data) {
            ...
            this.media.get_audio_stream(song_key).then((audio_source_url) => {
                ...
                this.load_audio(audio_source_url, 'm3u8');

                this.audio_data.current.audio_source = audio_source_url;
                this.audio_data.current.source_type = 'm3u8';
            });
            ...
            try {
                ...
                if(song_data) {
                    ...
                    if(song_data.downloaded && song_data.download_audio_blob) {
                        ...
                        this.load_audio(blob, 'blob');

                        this.audio_data.current.audio_source = blob;
                        this.audio_data.current.source_type = 'blob';
                    }

                    // allow original stream to load if not downloaded
                    return;
                }
        ...
        if(song_data.downloaded && song_data.download_audio_blob) {
            ...
            // using stored url
            this.load_audio(song_data.url.audio, 'm3u8');

            this.audio_data.current.audio_source = song_data.url.audio;
            this.audio_data.current.source_type = 'm3u8';
        }
        else {
            this.media.get_audio_stream(song_key).then((audio_source_url) => {
    ...
    public async skip_to_next(force: boolean = false): Promise<void> {
        ...
        const next_song_key = this.playlist.queue.shift();
    ...
    public load_playlist(identifier: Song_Playlist_Identifier | null, data: Song_Playlist | null, preserve_history: boolean = false, auto_play: boolean = false): void {
        ...
        Promise.all(this.playlist.queue.map(song_key => {
    ...
    // private use_silent_audio_on_ios_to_preserve_audio_pipeline: boolean = true;
    // private is_in_background: boolean = false;
    // private restoring_real_audio: boolean = false;
    // private want_to_play_after_silent_audio: boolean = false; // Track if user wants to be playing when switching to silent audio
    // private real_audio_timestamp: number = 0; // To track real audio time when using silent audio
    // private real_audio_duration: number = 0; // To track real audio duration when using silent audio
    ...
    //     if (!this.audio_element) {
    //         console.warn('switchToSilentAudio: no audio element');
    //         return;
    //     }

    //     try {
    //         this.real_audio_duration = this.audio_element.duration;
    //         this.real_audio_timestamp = this.audio_element.currentTime;

    //         console.log(`Switching to silent audio`);

    //         this.want_to_play_after_silent_audio = !this.audio_element.paused;
    ...
    //             try {
    //                 this.hls.stopLoad();
    //                 console.log('Detaching HLS media for silent audio switch...');
    //                 if(this.hls.media) this.hls.detachMedia();
    //             } catch (error) {
    //                 console.error('Error detaching HLS media:', error);
    //             }
    //         }

    //         // console.log('Pausing real audio before switching to silent audio...');

    //         const silent_audio_url = '/music/audio/silent.mp3';
    //         this.audio_element.src = silent_audio_url;
    //         this.audio_element.load();
    ...
    //             this.update_playback_state();
    //         }).catch((err) => {
    //             console.warn('Could not autoplay silent audio (user gesture required?):', err);
    //         });
    //     } catch (error) {
    //         console.error('Error switching to silent audio:', error);
    //     }
    // }

    // private async restore_real_audio(immediate_play: boolean = false): Promise<void> {
    //     if (!this.audio_element) {
    //         console.warn('restoreRealAudio: no audio element');
    //         return;
    //     }

    //     console.log(`Restoring real audio from ${this.media.song_key(this.current_song)} at ${this.real_audio_timestamp}s`);

    //     // if(immediate_play) this.load_and_play_track(this.media.song_key(this.current_song), this.current_song_data);
    //     // else this.load_track(this.media.song_key(this.current_song), this.current_song_data);
    //     this.load_audio(this.media.song_key(this.current_song), this.current_song_data);

    //     this.want_to_play_after_silent_audio = false;

    //     // this.audio_element.src = url;
    //     // this.audio_element.load();

    //     // this.audio_element.play().then(() => {
    //     //     console.log('🔊 Real audio playing');
    //     //     this.update_playback_state();
    //     // }).catch((err) => {
    //     //     console.warn('Could not autoplay real audio (user gesture required?):', err);
    //     // });
    // }
    ...
    //         };
    //         const handle_foreground = () => {
    //             this.is_in_background = false;

    //             this.restore_real_audio(this.want_to_play_after_silent_audio).catch(error => {
    //                 console.warn('Error restoring real audio on foreground:', error);
    //             });
    //         };
    ...
    //         });
    //         window.addEventListener('pagehide', (e) => {
    //             // pagehide can mean the page is being backgrounded or unloaded
    ...
    //         // Focus/blur as extra signals (some PWAs behave differently)
    //         window.addEventListener('blur', () => {
    //             // don't immediately treat blur as background — use debounced path
    ...
    //         if (this.loading) {
    //             navigator.mediaSession.playbackState = 'none';
    //         } else if (this.audio_element.paused || this.want_to_play_after_silent_audio) {
    ...
    //     if(this.is_in_background && this.is_ios_safari() && this.use_silent_audio_on_ios_to_preserve_audio_pipeline) {
    //         // switch back to real audio from silent audio
    //         this.restore_real_audio();
    //         return;
    //     }
    ...
    //     console.log('Current audio source before pause:', this.audio_element.src);
    //     console.log('Audio element paused state before pause():', this.is_in_background);
    //     if(this.is_in_background /*&& this.is_ios_safari()*/ && this.use_silent_audio_on_ios_to_preserve_audio_pipeline) {
    //         // If in background on iOS, switch to silent audio to keep audio pipeline alive
